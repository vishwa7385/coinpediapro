...
const CoinInfo = ({ coin }) => {
  ...
  const fetchHistoricalData = async () => {
    ...
    const formattedData = {
      ...
      }), // Assuming the timestamp is at index 0 in each priceData item
      datasets: [
        {
          label: `Price ( Past ${days} Days ) in ${currency}`,
          data: data.prices.map((priceData) => priceData[1]), // Assuming the price is at index 1 in each priceData item
          borderColor: "#EEBC1D",
        },
      ],
